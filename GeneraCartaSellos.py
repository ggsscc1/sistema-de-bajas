...
def GeneraCarta(claveUnica):
    ...
    pdf_filename = f"Carta de sellos {claveUnica}.pdf"  # Puedes cambiar el nombre del archivo según tus necesidades
    ...
    fecha_actual = date.today().strftime("San Luis Potosí, SLP a día:  %d  de   %B  de   %Y")
    ...
    pdf.drawString(letter[0]/2 - dif, letter[1]-3.25*inch, f"con clave única   {resultados[0][1]}    generación   {resultados[0][8]}   se encuentra en la carrera de")
    pdf.drawString(letter[0]/2 - dif, letter[1]-3.5*inch, f"Ingeniería  en  {resultados[0][7]}  y no tiene adeudos pendientes, presentamos a su")
    pdf.drawString(letter[0]/2 - dif, letter[1]-3.75*inch, f"nombre o algún asunto a tratar dentro de los laboratorios o con profesores.")
    ...
    pdf.setFont("Helvetica", 12)
    pdf.drawString(letter[0]/2-dif, letter[1]-6.5*inch, f"Correo electrónico:                   {resultados[0][5]}")
    ...
    if resultados[0][14] is not None:
        ...
        materia3 = ""
    pdf.drawString(letter[0]/2-dif, letter[1]-6.75*inch, f"Materia más difícil:                    {materia1}")
    ...
    if resultados[0][18] is not None:
        ...
        empresa = "No aplica"
    pdf.drawString(letter[0]/2-dif, letter[1]-7.75*inch,  f"Si trabaja, cuál es la empresa:  {empresa}")
    ...
    pdf.drawString(letter[0]/2-dif, letter[1]-8*inch,  f"Forma de titulación:                   {ftitulacion}")
    ...
    pdf.drawString(letter[0]/2-dif, letter[1]-8.25*inch,  f"Si es EGEL(S/N):  {esEgel}       Fecha de presentación de examen: {fechaEgel} ")
    ...
    pdf.drawString(letter[0]/2-dif, letter[1]-9.25*inch,  f"Para cualquier otro trámite que el alumno disponga, se da fe de él(la) solicitante")
    pdf.drawString(letter[0]/2-dif, letter[1]-9.5*inch,  f"esta ausente de las responsabilidades con los laboratorios del presente documento")
